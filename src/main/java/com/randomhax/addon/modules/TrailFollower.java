...
import com.randomhax.addon.RandomHax; // <-- important
import com.randomhax.addon.Utils;
...
import static com.randomhax.addon.Utils.positionInDirection;
...
public class TrailFollower extends Module {
    ...
    public TrailFollower() {
        super(RandomHax.CATEGORY, "TrailFollower", "Automatically follows trails in all dimensions.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (followingTrail && System.currentTimeMillis() - lastFoundTrailTime > trailTimeout.get()) {
            ...
            switch (trailEndBehavior.get()) {
                case DISABLE -> this.toggle();
                case FLY_TOWARDS_YAW -> targetYaw = trailEndYaw.get();
                case DISCONNECT -> mc.player.networkHandler.onDisconnect(new DisconnectS2CPacket(Text.literal("[TrailFollower] Trail timed out.")));
            }
        }
        ...
        switch (followMode) {
            ...
            case YAWLOCK -> mc.player.setYaw(Utils.smoothRotation(getActualYaw(mc.player.getYaw()), targetYaw, rotateScaling.get()));
        }
    }
    ...
    private boolean isValidChunk(ChunkPos pos, RegistryKey<World> dim) {
        PaletteNewChunks pnc = ModuleManager.getModule(PaletteNewChunks.class);
        boolean is119 = pnc.isNewChunk(pos.x, pos.z, dim);
        boolean is112 = ModuleManager.getModule(OldChunks.class).isOldChunk(pos.x, pos.z, dim);
        boolean isHighlighted = is119 || pnc.isInverseNewChunk(pos.x, pos.z, dim);
        return isHighlighted && ((!is119 && !only112.get()) || is112);
    }
    ...
    private void log(String msg) {
        info(msg);
        // webhook calls removed as requested
    }
