...
public class AutoPortal extends Module {
    ...
    public AutoPortal() {
        super(RandomHax.CATEGORY, "AutoPortal", "For the Base Hunter who has places to be.");
    }
    ...
    public void onActivate() {
        ...
        if (obsidianCount < 10) {
            error("Not enough obsidian (need 10+).");
            toggle();
            return;
        }
        portalBlocks.clear();
        ...
        int found = 0;
        for (BlockPos p : checkPositions) if (mc.world.getBlockState(p).getBlock().asItem() == Items.OBSIDIAN) found++;
        if (found >= checkPositions.size()) {
            error("Portal already exists here.");
            toggle();
            return;
        }
        ...
        for (int i = 1; i <= 3; i++) portalBlocks.add(base.offset(right, 0).up(i));
        for (int i = 1; i <= 3; i++) portalBlocks.add(base.offset(right, 3).up(i));
